...
function main() {
  console.log('🔧 Setting up Node-RED Gamification UI...');
  ...
  if (!projectRoot) {
    console.log('📦 Node-RED Gamification UI installed successfully!');
  ...
  const isTargetProject = packageJson.name === 'state-machine-mcp-driver' || 
                         packageJson.dependencies?.['threejs-gamification-ui'] ||
                         packageJson.devDependencies?.['threejs-gamification-ui'] ||
  ...
  if (!packagePath) {
    console.log('⚠️  Could not locate node-red-gamify-ui package path');
    return;
  }
  ...
  const targetPath = path.join(publicTemplatesDir, 'node-red-gamify-ui');
  
  console.log(`📦 Copying Node-RED UI assets from package to ${targetPath}`);
  ...
  try {
    ...
    // Copy dist to public_templates/node-red-gamify-ui
    copyDirectory(distPath, targetPath);
    
    console.log('✅ Node-RED Gamification UI setup completed successfully!');
    ...
    console.error('❌ Error during Node-RED UI setup:', error.message);
  ...
  while (currentDir !== path.dirname(currentDir)) { // not root
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (packageJson.name !== 'node-red-gamify-ui') {
          return currentDir;
        }
      } catch (error) {
        // Continue searching
...
function findOurPackage(projectRoot) {
  ...
  const possiblePaths = [
    path.join(projectRoot, 'node_modules', 'node-red-gamify-ui'),
    path.join(projectRoot, 'node_modules', '@escrivivir', 'node-red-gamify-ui'),
    path.join(projectRoot, 'node_modules', 'node-red-alephscript-sdk'),
  ];
